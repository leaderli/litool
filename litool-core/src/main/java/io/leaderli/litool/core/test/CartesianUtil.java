...
public class CartesianUtil {
    ...
    private static <T> Object[] cartesian(Class<T> type,
                                          AnnotatedElement annotatedElement,
                                          CartesianContext context) {

        return ReflectUtil.findAnnotationsWithMark(annotatedElement, Valuable.class)
                .filter(an -> {

                    Class<? extends CartesianFunction<?, ?>> value =
                            an.annotationType().getAnnotation(Valuable.class).value();
                    if (value == ObjectCartesian.class) {
                        return true;
                    }
                    Class<?> cartesianFunction2GenericType =
                            ReflectUtil.getGenericInterfacesType(value,
                ...
                .map(an -> {
                            ...
                            if (an instanceof ObjectValues) {
                                ...
                                return new CartesianObject<>(type,
                                        field -> CartesianUtil.cartesian(field,
                                                context)).cartesian().toArray();
                            }
    ...
    public static Object[] cartesian(Parameter parameter,
                                     CartesianContext context) {
    ...
    public static <T> Lira<T> cartesianByTemplate(Class<T> cls, Map<String,
            Object> template) {
        ...
        CartesianObject<T> cartesianObject = new CartesianObject<>(cls, field -> {
            ...
            if (val instanceof Iterable) {
                Object[] objects =
                        Lira.of((Iterable<?>) val).map(item -> convertToType(item
                                , type)).toArray();
                if (objects.length > 0) {
                    result = objects;
                }
                ...
                Object[] objects =
                        Lira.of(ClassUtil.toArray(val)).map(item -> convertToType(item, type)).toArray();
                if (objects.length > 0) {
                    result = objects;
                }
            } else {
