...
public class LiFileNameUtil {
	...
	public static String getName(String filePath) {
		...
		if (LiCharUtil.isFileSeparator(filePath.charAt(len - 1))) {
			// 以分隔符结尾的去掉结尾分隔符
			len--;
		}
		...
		for (int i = len - 1; i > -1; i--) {
			c = filePath.charAt(i);
			if (LiCharUtil.isFileSeparator(c)) {
				// 查找最后一个路径分隔符（/或者\）
				begin = i + 1;
				break;
			}
	...
	public static String extName(String fileName) {
		...
		if (index == -1) {
			...
			if (LiStringUtils.containsAny(substr, SPECIAL_SUFFIX)) {
				return substr;
			}

			String ext = fileName.substring(index + 1);
			// 扩展名中不能包含路径相关的符号
			return LiStringUtils.containsAny(ext, UNIX_SEPARATOR, WINDOWS_SEPARATOR) ? StrPool.EMPTY : ext;
		}
	}
	...
	public static String mainName(String fileName) {
		...
		if (LiCharUtil.isFileSeparator(fileName.charAt(len - 1))) {
			len--;
		}
	...
	public static boolean isType(String fileName, String... extNames) {
		return LiStringUtils.equalsAnyIgnoreCase(extName(fileName), extNames);
	}
	// -------------------------------------------------------------------------------------------- name end
}
