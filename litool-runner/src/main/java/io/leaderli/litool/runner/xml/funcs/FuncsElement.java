...
public void end_check(List<String> parseErrorMsgs) {
    ...
    for (FuncElement funcElement : funcList.lira()) {
        ...
        List<String> map = funcElement.getParams().lira()
                ...
                .map(paramName -> String.format("error reference [%s]:[%s] can only use func ref defined before this" +
        ...
        SaxEventHandler.addErrorMsgs(parseErrorMsgs, labelSet.add(funcElement.getLabel()), String.format("duplicate " +
