...
public class CoordinateElement extends BaseElement<CoordinateElement, CoordinateElementExecutor> {
    ...
    public void end_check(List<String> parseErrorMsgs) {
        ...
        SaxEventHandler.addErrorMsgs(parseErrorMsgs, tdElements.size() > 1, String.format("coordinate td should have " +
                "two " +
                "or more %s", id));
        SaxEventHandler.addErrorMsgs(parseErrorMsgs, size == 1, String.format("coordinate td should have same size %s",
                id));
        SaxEventHandler.addErrorMsgs(parseErrorMsgs, xSize == xLira.size() - 1, String.format("x-coordinate should " +
                "not " +
                "repeated %s", id));
        SaxEventHandler.addErrorMsgs(parseErrorMsgs, ySize == yLira.size() - 1, String.format("y-coordinate should " +
                "not " +
                "repeated %s", id));
    }
    ...
    private void assertExpression(VariablesModel model) {
        LiAssertUtil.assertTrue(model == VariablesModel.REQUEST || model == VariablesModel.RESPONSE || model == VariablesModel.LITERAL, "coordinate params " +
                "expression " +
