...
public void setName(String name) {
    LiAssertUtil.assertTrue(name.matches(LiConstant.ATTRIBUTE_NAME_RULE), String.format("the func name [%s] is not " +
...
public void setType(String type) {
    LiAssertUtil.assertTrue(TypeAlias.getALIAS().containsKey(type), String.format("the func type [%s] is unsupported",
            type));
    this.type = type;
}
...
public Lino<?> complexField(Class<?> parameterType, String value) {
    ...
    if (parameterType == Instruct.class) {
        ...
        return Lino.of(InstructContainer.getInnerMethodByAlias(value)).assertNotNone(String.format("instruct [%s] is " +
                "unsupported", value));
    }
