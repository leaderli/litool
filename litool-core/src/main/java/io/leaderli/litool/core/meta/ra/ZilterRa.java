 ...
 * @since 2022/6/27
 */
class ZilterRa<T> extends RaWithPrevPublisher<T> {


    private final Function<Lino<T>, ?> filter;


    public ZilterRa(PublisherRa<T> prevPublisher, Function<Lino<T>, ?> filter) {
        super(prevPublisher);
    ...
    private final class FilterSubscriberSubscription extends IntermediateSubscriberSubscription<T, T> {
        ...
        public void next(T t) {
            if (BooleanUtil.parse(filter.apply(Lino.of(t)))) {
