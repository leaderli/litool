...
//-----------------------------------------------------------------------
     ...
     * @since 2.0
     */
    public static String reverseDelimited(final String str, String separatorChar) {
        if (str == null) {
            return null;
        }
...
//            return out.toString();
    ...
    /**
     * 将字符串切割为多个小于最大长度额片段，根据指定的字符，若找不到合适的切割位置，则强制切割，但切割的字符不能为不可切割字符，会向前查找，不可切割字符需要归属于下一个片段
     *
     * @param text              原始字符
     * @param boundaryChars     切割的字符
     * @param lookBackSkipChars 不可切割字符
     * @param size              片段的最大长度
     * @param lookbackStep      强制切割时最多查找的步数，其需要小于等于size/4
     */
    public static String[] splitLessThanSize(String text, String boundaryChars, String lookBackSkipChars, int size, int lookbackStep) {
        LiAssertUtil.assertTrue(size > 4 && size / 4 >= lookbackStep, "size > 4 && size / 4 >= lookbackStep ");
        if (text == null) {
            return new String[]{""};
        }
        ...
        while (text.length() >= size) {
            ...
            if (notFind) {
                ...
                for (; end >= size - lookbackStep; end--) {
                    char c = text.charAt(end - 1);
                    if (lookBackSkipChars.indexOf(c) < 0) {
                        result.add(text.substring(0, end));
                        text = text.substring(end);
                        notFind = false;
                        break;
