...
public class DatePlusInstruct implements Instruct {
...
public String invoke(String dateStr, DateUnitEnum unit, Integer gap) {
    if (dateStr.trim().length() != 8) {
        throw new IllegalArgumentException(String.format("argument dateStr:[%s] is illegal of Instruct [%s]", dateStr
                , name()));
    }
    DateTimeFormatter yyyyMMdd = DateTimeFormatter.ofPattern("yyyyMMdd");
    LocalDate date = LocalDate.parse(dateStr, yyyyMMdd);

    LocalDate plusResult = unit.plus(date, gap);
